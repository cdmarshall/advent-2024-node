...
// Check if array is already in asc or desc order, if so confirm that difference between every two meets requirements.
...
// Iterate through an unsafe array and remove current index. Check safety of new array.
function problemDampener(numbers) {
  for(let i = 0; i < numbers.length; i++) {
    if(checkSafety([...numbers.slice(0, i), ...numbers.slice(i + 1)])) return true;
  }
  return false;
}
